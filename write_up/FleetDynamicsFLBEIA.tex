...
incompatibilities in quotas for stocks caught as part of mixed fisheries. To
support the move away from single stock management towards a multi-stock
approach requires scientific tools to evaluate how a management measure impacts
all the stocks caught together in the fishery. Such a mixed-fishery approach
necessitates modelling framework evaluating management strategies to not only
take account of existing technical interactions, but how short- and long-term
decisions making by fishers might affect the development of those interactions
over time, and thus how such decisions might affect management outcome. \\

The location choice models implemented in the FLBEIA framework range from a
simple process-based gravity model to more complex RUM and Markov transition
models but all are flexible, providing the user with the ability to tailor the
model to the specificities of the fishery. At present, the possible covariates
include stock specific catch rates as well costs, a seasonal effect and past
share spent in the métier but the framework is easily extendable by using the
covariates input to FLBEIA. \\

We implement three different approaches to modelling location choice within a
mixed-fishery management strategy evaluation framework and demonstrate how it
can be used to provide alternative hypotheses through fleet operating models.
We argue that inclusion of location choice models in a mixed-fishery MSE is
essential to consider the potential impact of fleet dynamics on management
outcomes. While no single model may provide the `best' approach, a range of
models could be included to evaluate these hypotheses and find solutions that
meet the overall objectives. This is similar to how uncertainty about the form
of recruitment dynamics is included in single stock MSEs
\citep[e.g.][]{ICES2020}. \\
...
grounds in a gravity, RUM and Markov model. Further, we show through simulation
how this leads to different allocations of effort across the métier and how
this effects management outcome. While our MSE was only implemented as a
