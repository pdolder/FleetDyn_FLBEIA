...
S <- "COD"
plot(FLStocks(base = base[["stocks"]][[S]][,ac(2015:2020)], 
	      gravity = gravity[["stocks"]][[S]][,ac(2015:2020)],
	      rum = rum[["stocks"]][[S]][,ac(2015:2020)])) + 
	    theme(legend.position = "top") + ggtitle(paste("Comparison", S))

bio <- rbind(
      cbind(sc = "base", bioSum(base, long = FALSE, years = ac(2016:2022))),
cbind(sc = "gravity", bioSum(gravity, long = FALSE, years = ac(2016:2022))),
cbind(sc = "rum", bioSum(rum, long = FALSE, years = ac(2016:2022)))
)
...
advice <- rbind(
      cbind(sc = "base", advSum(base, long = FALSE, years = ac(2016:2022))),
      cbind(sc = "gravity", advSum(gravity, long = FALSE, years = ac(2016:2022))),
      cbind(sc = "rum", advSum(rum, long = FALSE, years = ac(2016:2022)))
      )
