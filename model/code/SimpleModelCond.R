...
save(fleets,file=file.path(in.path,'FLFleetsExt_expanded.RData'))
...
load(file.path(in.path,'sr_fits.RData'))
...
COD_params.array<-array(dim=c(COD_params.n, ny, ns=ns, ni=ni), dimnames=list(param=c("a","b"), year=as.character(first.yr:last.yr), season=as.character(1), iter=as.character(1:ni)))

COD_params.array[1,,,]<-params(COD_sr)[1]
COD_params.array[2,,,]<-params(COD_sr)[2]
COD_params.name     <- c('a','b') 

## multiplicative uncertainty
unit.flq<-array(1,dim=c(1, ny, ns=ns, ni=ni), dimnames=list(age=as.character(1), year=as.character(first.yr:last.yr), season=as.character(1), iter=as.character(1:ni)))

COD_uncertainty.flq <- FLQuant(exp(rnorm(length(first.yr:last.yr)*ni,mean=0,
                                            sd(residuals(COD_sr)))),dimnames=list(age='1',year=first.yr:last.yr,iter=1:ni))     
...
COD_age.max<-COD_range.max<-COD_range.plusgroup<-as.numeric(dimnames(n(biols[["COD_NS"]]))$age[length(dimnames(n(biols[["COD"]]))$age)])
...
HAD_params.array<-array(dim=c(HAD_params.n, ny, ns=ns, ni=ni), dimnames=list(param=c("a","b"), year=as.character(first.yr:last.yr), season=as.character(1), iter=as.character(1:ni)))

HAD_params.array[1,,,]<-params(HAD_sr)[1]
HAD_params.array[2,,,]<-params(HAD_sr)[2]
...
HAD_uncertainty.flq <- FLQuant(exp(rnorm(length(first.yr:last.yr)*ni,mean=0,
                                            sd(residuals(HAD_sr)))),dimnames=list(age='1',year=first.yr:last.yr,iter=1:ni))     
...
HAD_age.max<-HAD_range.max<-HAD_range.plusgroup<-as.numeric(dimnames(n(biols[["HAD_NS"]]))$age[length(dimnames(n(biols[["HAD"]]))$age)])
...
## 
stks.data <- list(COD_NS =ls(pattern="^COD_NS"),
                  HAD =ls(pattern="^HAD"),
                  POK =ls(pattern="^POK"),
                  WHG_NS =ls(pattern="^WHG_NS"),
                  PLE_NS =ls(pattern="^PLE_NS"),
                  SOL_NS =ls(pattern="^SOL_NS"),
                  PLE_EC =ls(pattern="^PLE_EC"),
                  SOL_EC =ls(pattern="^SOL_EC")
                                    ) 
