...
## Updated functions
source(file.path("..", "..","tests", "OM_2a_Effort_Dynamics_SMFB_VarEffortShare.R"))
...
fleets.ctrl[['IE_Otter']][['effshare.model']] <- 'rum'
fleets.ctrl[['IE_Otter']][['RUM.fit']] <- RUM_model_fit

library(mlogit)
#library(nnet)

debug(rum)
debug(make_RUM_predict_df)
debug(update_RUM_params)
debug(predict_RUM)
